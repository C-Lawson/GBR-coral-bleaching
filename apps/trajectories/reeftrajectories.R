...
# rsconnect::deployApp("/Users/rof011/GBR-coral-bleaching/apps/trajectories/", appPrimaryDoc="reeftrajectories.R", appName="DHWtrajectories", appTitle="DHWtrajectories")
...
df <- readRDS("gbr_shape_dhw.RDS") |>
  ...
  st_set_geometry(NULL) %>%  # Remove geometry column if you don't need it
  as.data.frame() |>
  mutate(reef_id = paste0(as.character(gbr_name), " (", id,")")) |>
  dplyr::select(-id, -gbr_name) |>
  pivot_longer(-c(reef_id, latitude),  names_to="year", values_to="dhw_max") |>
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      tags$p(""),
      tags$p(""),
      materialSwitch("show_error", "Show Error", FALSE, status="danger"),
            width=3
    ),
    mainPanel(
...
server <- function(input, output, session) {
  ...
  output$tempPlot <- renderPlotly({
    ...
    if(is.null(input$reef_id) || length(input$reef_id) == 0) {
      df2 <- data.frame(year = 0, dhw_max = 0)
      placeholder_plot <- ggplot(data = df2, aes(x = year, y = dhw_max)) +
        theme_bw() +
      ...
      return(ggplotly(placeholder_plot))
    } else {
    ...
    theme(plot.title = element_text(size = 12),
        ...
        geom_smooth(data=df, aes(color=reef_id, x = year, y = dhw_max),
                     method = "gam", formula = y ~ s(x, bs = "cr", k = 10), linewidth=0.8, #color="darkred",
