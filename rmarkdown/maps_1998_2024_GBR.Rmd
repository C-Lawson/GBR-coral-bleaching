...
```
Synchronised spatial maps of the 1998, 2002, 2016, 2017, 2020, 2022, 2024 mass coral bleaching events (click on individual reefs for `reef_name` / `label_ID`). Note that the GBR shp file was downsampled using `st_simplify` to reduce file sizes and leads to a coarser resolution map appearance:
...
map_outputs <- gbr_shape_dhw_simple_long |> dplyr::filter(year %in% c("1998","2002","2012",
                                                    "2016","2017","2020",
                                                    "2022","2024"))
...
#tm_basemap("Esri.WorldImagery") +
  tm_shape(map_outputs) +
    tm_polygons(col="dhw_max",  
              title="max DHW", 
              palette = dhw_scale,
              style = "fixed",   
              #labels = labels,
